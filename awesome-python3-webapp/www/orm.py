...
#@asyncio.coroutine. python3已弃用该写法，改用async await
async def create_pool(loop, **kw):
    logging.info('create database connection pool...')
    global __pool   #global 声明**为全局变量
...
#     global __pool
#
#     with (await __pool) as conn:#改用async&await写法之后，该部分代码不适用yield，协程调用语句yield from临时替换成await
...
#             rs = await cur.fetchall()
#         await cur.close()
#         logging.info('rows returned: %s' % len(rs))#返回了多少行记录
#         return rs
        ...
        for k,v in attrs.items():
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    ...
                    fields.append(k)
        if not primaryKey:
            raise StandardError('Primary ket not found.')
